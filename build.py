...
def configure_and_build(ui: UI, cfg: BuildConfig, target: str | None = None) -> tuple[Path, Path]:
    ...
    preferred = cfg.artifact or target or cfg.project_name
...
def build_selected_artifact(ui: UI, cfg: BuildConfig) -> BuiltArtifact:
    elf, bin_path = configure_and_build(ui, cfg)
    return BuiltArtifact("firmware", bin_path, "0x08000000", elf)
...
def make_parser() -> argparse.ArgumentParser:
    ...
    f.add_argument("--addr", default=None,
                   help="Override the flash address (default: 0x08000000).")
...
def main() -> None:
    ...
    if args.cmd == "build":
        build_selected_artifact(ui, cfg)
        return

    if args.cmd == "flash":
        artifact = build_selected_artifact(ui, cfg)
